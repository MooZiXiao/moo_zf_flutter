...
import 'package:moo_zf_flutter/widget/common_check_button.dart';
      ...
      child: Wrap(
        ...
        children: list.map((item) =>
          GestureDetector(
            onTap: () {
              setState(() {
                list = list.map((innerItem) => innerItem == item ? RoomApplianceItem(item.title, item.iconPoint, !item.isChecked) : innerItem).toList();
              });
              if(null != widget.onChange) widget.onChange(list);
            },
